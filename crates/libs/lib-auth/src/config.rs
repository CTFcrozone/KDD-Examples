use lib_utils::envs::{get_env_b64_b64u_as_u8s, get_env_parse};
...
impl AuthConfig {
	fn load_from_env() -> lib_utils::envs::Result<AuthConfig> {
		Ok(AuthConfig {
			// -- Crypt
			PWD_KEY: get_env_b64_b64u_as_u8s("SERVICE_PWD_KEY")?,

			TOKEN_KEY: get_env_b64_b64u_as_u8s("SERVICE_TOKEN_KEY")?,
