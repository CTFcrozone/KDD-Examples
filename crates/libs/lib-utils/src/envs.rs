use crate::b64::{b64_decode_to_string, b64u_decode};
use std::env;
use std::str::FromStr;
...
pub fn get_env_b64_b64u_as_u8s(name: &'static str) -> Result<Vec<u8>> {
	let b64u_encoded = b64_decode_to_string(&get_env(name)?)
		.map_err(|_| Error::WrongFormat(name))?;
	b64u_decode(&b64u_encoded).map_err(|_| Error::WrongFormat(name))
}
